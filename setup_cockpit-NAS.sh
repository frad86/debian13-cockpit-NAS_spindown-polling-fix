...
tee /etc/udev/rules.d/69-udisks-no-polling.rules > /dev/null <<'EOF'
...
sed -i 's/^DEVICESCAN.*/DEVICESCAN -n standby,15,q/' /etc/smartd.conf
...
sed -i 's/START_HD_IDLE=false/START_HD_IDLE=true/' /etc/default/hd-idle
[[ -s /etc/default/hd-idle && "$(tail -c1 /etc/default/hd-idle | wc -l)" -eq 0 ]] && echo '' | tee -a /etc/default/hd-idle
echo 'HD_IDLE_OPTS="-i 0 -a sda -i 60 -c ata -a sdb -i 60 -c ata -a sdc -i 60 -c ata"' | tee -a /etc/default/hd-idle
